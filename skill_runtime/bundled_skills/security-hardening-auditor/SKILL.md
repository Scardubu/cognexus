...
description: 'Audits and hardens full-stack Next.js and Node.js applications against OWASP Top 10: authentication with Auth.js v5, authorization patterns, Content Security Policy headers, API rate limiting, CORS configuration, input validation, SQL injection prevention, XSS protection, CSRF defense, and secrets management. Use when a task requires security hardening auditor analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.'
license: MIT
...
**Use this skill when:** Audits and hardens full-stack Next.js and Node.js applications against OWASP Top 10: authentication with Auth.js v5, authorization patterns, Content Security Policy headers, API rate limiting, CORS configuration, input validation, SQL injection prevention, XSS protection, CSRF defense, and secrets management. Use when a task requires security hardening auditor analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.
...
- "Audit this implementation using the security hardening auditor workflow and return prioritized findings."
- "Design a production-ready security hardening auditor solution for the attached codebase with exact file paths and tests."
- "Use security-hardening-auditor to review the current approach, identify risks, and propose the smallest safe upgrade."
