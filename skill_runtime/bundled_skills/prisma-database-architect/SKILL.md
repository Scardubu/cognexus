...
description: Designs, audits, and optimizes Prisma schemas, migrations, queries, and connection pooling for production PostgreSQL deployments. Use when a task requires prisma database architect analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.
license: MIT
...
**Use this skill when:** Designs, audits, and optimizes Prisma schemas, migrations, queries, and connection pooling for production PostgreSQL deployments. Use when a task requires prisma database architect analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.

**Do not use it when:** a straightforward answer or a smaller deterministic tool can safely complete the task. Do not perform irreversible changes or assume missing architecture, data, dependencies, or deployment state.
...
- "Audit this implementation using the prisma database architect workflow and return prioritized findings."
- "Design a production-ready prisma database architect solution for the attached codebase with exact file paths and tests."
- "Use prisma-database-architect to review the current approach, identify risks, and propose the smallest safe upgrade."
