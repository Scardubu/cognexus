...
description: 'Surgically audits and generates tsconfig.json, ESLint flat config, Prettier config, and their integration for TypeScript strict mode projects. Diagnoses and fixes common misconfigurations: wrong moduleResolution, path alias issues, slow type-checking, ESLint and Prettier conflicts, and monorepo project references. Use when a task requires typescript config surgeon analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.'
license: MIT
...
**Use this skill when:** Surgically audits and generates tsconfig.json, ESLint flat config, Prettier config, and their integration for TypeScript strict mode projects. Diagnoses and fixes common misconfigurations: wrong moduleResolution, path alias issues, slow type-checking, ESLint and Prettier conflicts, and monorepo project references. Use when a task requires typescript config surgeon analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.
...
- "Audit this implementation using the typescript config surgeon workflow and return prioritized findings."
- "Design a production-ready typescript config surgeon solution for the attached codebase with exact file paths and tests."
- "Use typescript-config-surgeon to review the current approach, identify risks, and propose the smallest safe upgrade."
