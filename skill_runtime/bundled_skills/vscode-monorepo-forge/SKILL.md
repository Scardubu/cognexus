...
description: 'Generates complete VS Code multi-root workspace configurations for monorepos: .code-workspace files, per-package settings.json, launch.json debug configs, tasks.json, and turbo.json pipeline definitions. Use when a task requires vscode monorepo forge analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.'
license: MIT
...
**Use this skill when:** Generates complete VS Code multi-root workspace configurations for monorepos: .code-workspace files, per-package settings.json, launch.json debug configs, tasks.json, and turbo.json pipeline definitions. Use when a task requires vscode monorepo forge analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.
...
- "Audit this implementation using the vscode monorepo forge workflow and return prioritized findings."
- "Apply the vscode monorepo forge workflow in dry-run mode first, then provide exact commands and rollback steps."
- "Use vscode-monorepo-forge to review the current approach, identify risks, and propose the smallest safe upgrade."
