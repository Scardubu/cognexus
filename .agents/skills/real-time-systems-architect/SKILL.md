...
description: 'Designs and implements production-grade real-time systems: WebSocket and SSE architectures, presence/collaboration patterns, optimistic UI with rollback, BullMQ job progress streaming, SwarmX agent status feeds, and race condition handling. Use when a task requires real time systems architect analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.'
license: MIT
...
**Use this skill when:** Designs and implements production-grade real-time systems: WebSocket and SSE architectures, presence/collaboration patterns, optimistic UI with rollback, BullMQ job progress streaming, SwarmX agent status feeds, and race condition handling. Use when a task requires real time systems architect analysis, design, implementation, audit, debugging, or production hardening, even when the skill name is not mentioned.
...
- "Audit this implementation using the real time systems architect workflow and return prioritized findings."
- "Design a production-ready real time systems architect solution for the attached codebase with exact file paths and tests."
- "Use real-time-systems-architect to review the current approach, identify risks, and propose the smallest safe upgrade."
